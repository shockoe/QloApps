...
When a PrestaShop order is created (either through the standard checkout or via the external API), a hook is triggered. The `hotelreservationsystem` module listens for this hook and automatically generates the corresponding `htl_booking_detail` records from the cart data. This ensures that every booking is precisely linked to its specific order line item, maintaining data integrity even in complex orders with multiple room types.
